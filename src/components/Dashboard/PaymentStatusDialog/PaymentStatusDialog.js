...
import { db, auth } from "../../shared/firebase"; // Ensure your Firebase setup is correctly imported
import "./paymentStatusDialog.css";
import ConfirmDeleteDialog from "../../Dashboard/ConfirmDeleteDialog/ConfirmDeleteDialog"; // Import the confirmation dialog
...
const PaymentStatusDialog = ({
  ...
  const renderStatusColor = (status) => {
    switch (status) {
      case "Done":
        return "#32cd32"; // Bright green for done
      case "Partial":
        return "#ffa500"; // Vibrant orange for partial
      case "Not Paid":
        return "#e74c3c"; // Bold red for not paid
      default:
  ...
  const handleDelete = async () => {
    ...
    try {
      const guestRef = db
        ...
        .doc(guest.id);
      await guestRef.update({ paymentDetails: [] });
      onPaymentUpdate({
        ...guest,
        paymentDetails: {}, // Clear payment details locally
      });
      if (typeof onSnackbarOpen === "function") {
          ...
          <div className="guest-details">
            {[
              ...
              { label: "Aadhar Number", value: guest.aadharNumber },
              {
                ...
                value: formatDate(guest.dateOfAdmission),
              },
              { label: "Permanent Address", value: guest.permanentAddress },
              {
                label: "Floor/Room/Type",
                value: `${guest.floorNo}/${guest.roomNo}/${guest.roomType}`,
              },
              { label: "Present Status", value: guest.presentStatus },
              { label: "Deposit Amount", value: `₹${guest.depositAmount}` },
              {
                label: "Maintenance Charges",
                value: `₹${guest.maintenanceCharges}`,
              },
              {
                label: "Full Deposit",
                value: guest.fullDeposit ? "Yes" : "No",
              },
            ].map(({ label, value }) => (
